...
export default function AdicionarManualmentePage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const hasEmptyFields = items.some(item => !item.sn || !item.tipo || !item.data || !item.ocomon);
    if (hasEmptyFields) {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold">Adicionar Itens ao Estoque</h1>
      </div>
          ...
          <div key={index} className="bg-card rounded-lg shadow-md p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  value={item.tipo}
                  onChange={(e) => updateItem(index, 'tipo', e.target.value as BatchItem['tipo'])}
                  className="w-full p-2 border rounded-md"
                  required
                >
